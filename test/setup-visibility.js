...
test("Setup enviroment", function (t) {
	...
	t.test("Should reflect settings on newly configured loggers:", function (t) {
		t.equal(
			log.getLevel("envtesta").getNs("e2:n").isEnabled, true,
			"Should override nested ns for existing ns parent"
		);
		t.equal(
			log.getLevel("envtesta").getNs("n1:d").isEnabled, false,
			"Shuld override nested ns on overriden ns"
		);
		t.equal(log.getLevel("envtesta").getNs("n1").isEnabled, true, "Should override ns");
		t.equal(
			log.getLevel("envtesta").getNs("n2:n:foo").isEnabled, true,
			"Should override nested ns deep way"
		);
		t.equal(
			log.getLevel("envtesta").getNs("n2:n").isEnabled, true, "Should override nested ns"
		);
		t.equal(
			log.getLevel("envtesta").getNs("n3").isEnabled, false, "Non overriden ns should inherit"
		);
		t.equal(
			log.getLevel("envtestb").getNs("n1:d").isEnabled, false,
			"Should override nested ns on non overriden logger"
		);
		t.equal(
			log.getLevel("envtestb").getNs("n2").isEnabled, false,
			"Should override ns on non overriden logger"
		);
		t.equal(
			log.getLevel("envtestb").getNs("n3").isEnabled, true,
			"Non overriden ns should be unaffected"
		);

		t.equal(
			log.getLevel("envtestc").getNs("foo:mar").isEnabled, false,
			"Should reflect overridance on overriden newly setup level, deep way"
		);
		t.equal(
			log.getLevel("envtestc").isEnabled, false,
			"Should reflect overridance on newly setup level"
		);

		t.equal(
			log.getLevel("envtestd").getNs("n1:foo").isEnabled, false,
			"Should reflect overridance on newly setup level, deep way"
		);
		t.equal(
			log.getLevel("envtestd").getNs("n1").isEnabled, false,
			"Should reflect overridance on newly setup level"
		);
		t.equal(
			log.getLevel("envtestd").isEnabled, true,
			"Nested overrides should not affect newly created level"
		);
		t.equal(
			log.getLevel("envtestd").getNs("n2").isEnabled, true,
			"Deep nested overrides should not affect ns  on newly created level"
		);
		t.equal(
			log.getLevel("envtestd").getNs("n2:elo").isEnabled, false,
