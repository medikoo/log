...
test("(main): Should provide enable/disable functionality on level/name config", function (t) {
	var log;
	...
	t.equal(typeof restore, "function", "disable() should return object with `restore` function");
	t.equal(log.get("enabletest").isEnabled, false, "Should be disabled after `disable()` call");

	t.equal(
		log.get("enabletest").get("foo").isEnabled, false, "New nested names should inherit setting"
	);
	restore();
	t.equal(log.get("enabletest").isEnabled, true, "`restore` should bring previous state");

	restore = log.get("enabletest").enable().restore;
	t.equal(typeof restore, "function", "enable() should return object with `restore` function");
	t.equal(log.get("enabletest").isEnabled, true, "Should remain enabled after `enable()` call");
